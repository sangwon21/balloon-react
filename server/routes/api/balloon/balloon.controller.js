...
exports.users = (req, res) => {
  User.find(function (err, users) {
    if (err) return res.status(500).json({ result: false, message: "정보 조회에 실패 했습니다." });
...
exports.user = (req, res) => {
  User.findById(req.decoded._id, function (err, user) {
    if (err) return res.status(500).json({ result: false, message: "정보 조회에 실패 했습니다." });
    if (!user) return res.status(404).json({ result: false, message: "유저를 찾을 수 없습니다." });
...
exports.picture = (req, res) => {
  User.findById(req.decoded._id, function (err, user) {
    ...
    user.save(function (err) {
      if (err) return res.status(500).json({ result: false, message: "사진 업데이트에 실패 했습니다." });
...
exports.message = (req, res) => {
  User.findById(req.decoded._id, function (err, user) {
    if (err) return res.status(500).json({ result: false, message: "정보 조회에 실패 했습니다." });
    if (!user) return res.status(404).json({ result: false, message: "유저를 찾을 수 없습니다." });
    if (!user.balloonSize) return res.status(404).json({ result: false, message: "사용 가능한 풍선이 부족합니다.", balloonSize: user.balloonSize });
    if (user.email === req.body.receiverEmail) return res.status(404).json({ result: false, message: "자신에게 메시지를 보낼 수 없습니다." });
    ...
    Message.find(
      ...
      function (err, messages) {
        ...
        message.save(function (err) {
          if (err) return res.status(500).json({ result: false, message: "메시지 전송에 실패 했습니다." });
          user.balloonSize--;
          user.save();
